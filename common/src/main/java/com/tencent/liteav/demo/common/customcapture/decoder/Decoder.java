package com.tencent.liteav.demo.common.customcapture.decoder;
...
public class Decoder extends ProvidedStage<Frame> {
    ...
    private       long                  mSkipFrameBeforeInThisLoop = 0;//这次循环的时候，忽略时间小于该时间的帧（处理Seek的时候，需要使用）
    ...
    public void setup() throws SetupException {
        ...
        if (mimeType.startsWith("video/") && null == outputSurface) {
    ...
    private void drainDecodedFrame() {
        ...
        final MediaFormat outputFormat = mMediaCodec.getOutputFormat();
        ...
        frame.width = outputFormat.getInteger(MediaFormat.KEY_WIDTH);
        frame.height = outputFormat.getInteger(MediaFormat.KEY_HEIGHT);

        // 忽略Seek之前的帧（同时不要忽略EOS帧）
        if (mSkipFrameBeforeInThisLoop > frame.presentationTimeUs && !MediaUtils.hasEosFlag(frame.flags)) {
    ...
    private void feedDataToMediaCodec() throws SetupException {
        ...
        if (mIsLooping && MediaUtils.hasEosFlag(bufferInfo.flags)) {
            mExtractor.restart();
