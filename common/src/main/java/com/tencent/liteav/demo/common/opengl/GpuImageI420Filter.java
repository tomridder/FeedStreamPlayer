...
public class GpuImageI420Filter extends GPUImageFilter {
    ...
    private       int    mYTextureId  = NO_TEXTURE;
    private       int    mUvTextureId = NO_TEXTURE;
    private final Size   mTextureSize = new Size(-1, -1);
    private       byte[] mYData;
    ...
    public void loadYuvDataToTexture(ByteBuffer byteBuffer, int width, int height) {
        updateTextureSize(width, height);
        byteBuffer.position(0);
        mYTextureId = OpenGlUtils.loadTexture(GLES20.GL_LUMINANCE, byteBuffer, width, height, mYTextureId);
        byteBuffer.position(width * height);
        mUvTextureId = OpenGlUtils.loadTexture(GLES20.GL_LUMINANCE, byteBuffer, width, height / 2, mUvTextureId);
    }
    ...
    private void updateTextureSize(int width, int height) {
        // 纹理大小发生变化，需要重新创建纹理
        if (mTextureSize.width != width || mTextureSize.height != height) {
